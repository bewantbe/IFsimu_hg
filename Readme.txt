...
程序默认会把完整的电平数据输出到 data 目录下的 staffsave.txt. 每次运行会覆盖上次数据
 如果需要输出放电时刻的数据, 需在执行时加上参数 --save-spike -
...
程序命令行选项:
  ...
  raster_tuning [选项]
    ...
    -s, --save-while-cal
                  设置边计算边输出电平数据(默认)
    --save-last   设置在最后输出电平数据(输出数据不完整), 但速度最快. (目前仅用作测试)
    ...
    --pr-mul [VALUE ...] [VALUE@POSITION ...]
                  在命令行设置某些神经元的 poisson 输入率. 可以只设定前几个的输入率.
                  可以用 VALUE@POSITION 的语法指定第几个的值, 编号从 1 开始.
                  未指定值的默认是 1. 必须提前指定神经元个数.
    --ps-mul [VALUE ...] [VALUE@POSITION ...]
                  在命令行设置某些神经元的 poisson 输入强度(兴奋型). 语法参见 --pr-mul
    --psi-mul [VALUE ...] [VALUE@POSITION ...]
                  在命令行设置某些神经元的 poisson 输入强度(抑制型). 语法参见 --pr-mul
